    ...
    return (
        <section className="flex flex-col gap-8 w-full font-Outfit mb-20 animate-in fade-in duration-500 px-4 lg:px-0">
            ...
            <div className="flex flex-col gap-2">
                <h1 className="text-2xl lg:text-3xl font-bold text-[#333333]">Batch Verification</h1>
            ...
            <div className="flex flex-col gap-10 lg:gap-12">
                {candidates.map((candidate, index) => (
                    <div key={candidate.id} className="flex flex-col gap-5 lg:gap-6 relative animate-in slide-in-from-top-4 duration-500" style={{ animationDelay: `${index * 100}ms` }}>
                        <div className="flex items-center justify-between">
                            <h2 className="text-base lg:text-lg font-bold text-[#333333]">Candidate {index + 1}</h2>
                            {index > 0 && (
                                <button 
                                    onClick={() => removeCandidate(candidate.id)}
                                    className="text-red-500 text-xs lg:text-sm font-bold hover:underline"
                                >
                                    Remove Candidate
                                </button>
                            )}
                        ...
                        <div className="bg-white border border-[#e6e6e7] rounded-[24px] lg:rounded-[32px] p-6 lg:p-10 shadow-sm flex flex-col gap-8 lg:gap-10">
                            ...
                            <div className="grid grid-cols-1 lg:grid-cols-2 gap-x-10 gap-y-6 lg:gap-y-8">
                                <div className="flex flex-col gap-2.5">
                                    ...
                                    <input type="text" placeholder="Enter First Name" className="w-full bg-[#fcfcfc] border border-[#f2f2f3] rounded-2xl h-12 lg:h-14 px-5 lg:px-6 focus:outline-none focus:ring-2 focus:ring-[#ff895d]/20 transition-all text-[#333333] font-medium placeholder:text-[#bebebe]" />
                                </div>
                                <div className="flex flex-col gap-2.5">
                                    <label className="text-[14px] lg:text-[15px] font-bold text-[#333333]">Last Name</label>
                                    <input type="text" placeholder="Enter Last Name" className="w-full bg-[#fcfcfc] border border-[#f2f2f3] rounded-2xl h-12 lg:h-14 px-5 lg:px-6 focus:outline-none focus:ring-2 focus:ring-[#ff895d]/20 transition-all text-[#333333] font-medium placeholder:text-[#bebebe]" />
                                </div>
                                <div className="flex flex-col gap-2.5">
                                    <label className="text-[14px] lg:text-[15px] font-bold text-[#333333]">Alias</label>
                                    <input type="text" placeholder="Enter Name" className="w-full bg-[#fcfcfc] border border-[#f2f2f3] rounded-2xl h-12 lg:h-14 px-5 lg:px-6 focus:outline-none focus:ring-2 focus:ring-[#ff895d]/20 transition-all text-[#333333] font-medium placeholder:text-[#bebebe]" />
                                </div>
                                <div className="flex flex-col gap-2.5">
                                    <label className="text-[14px] lg:text-[15px] font-bold text-[#333333]">Phone Number</label>
                                    <input type="text" placeholder="+234 |" className="w-full bg-[#fcfcfc] border border-[#f2f2f3] rounded-2xl h-12 lg:h-14 px-5 lg:px-6 focus:outline-none focus:ring-2 focus:ring-[#ff895d]/20 transition-all text-[#333333] font-medium placeholder:text-[#bebebe]" />
                                </div>
                            </div>
                            ...
                            <div className="flex flex-col gap-8">
                                {candidate.verifications.map((v, vIdx) => (
                                    <div key={v.id} className="border border-[#f2f2f3] rounded-[20px] lg:rounded-[24px] p-5 lg:p-8 flex flex-col gap-6 lg:gap-8 relative bg-[#f9fbff]/20">
                                        {vIdx > 0 && (
                                            <button 
                                        ...
                                        {v.type === "Address Verification" && (
                                            <div className="grid grid-cols-1 gap-6 animate-in slide-in-from-top-2">
                                                <div className="flex flex-col gap-2.5">
                                                    ...
                                                    <input type="text" placeholder="Enter address" className="w-full bg-white border border-[#f2f2f3] rounded-2xl h-12 lg:h-14 px-5 focus:outline-none" />
                                                </div>
                                                <div className="flex flex-col gap-2.5">
                                                    <label className="text-[14px] font-bold text-[#666666]">Landmark</label>
                                                    <input type="text" placeholder="Enter landmark" className="w-full bg-white border border-[#f2f2f3] rounded-2xl h-12 lg:h-14 px-5 focus:outline-none" />
                                                </div>
                                            </div>
                                        )}

                                        {v.type === "Guarantor's Verification" && (
                                            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 lg:gap-8 animate-in slide-in-from-top-2">
                                                <div className="flex flex-col gap-2.5">
                                ...
                                <button 
                                    ...
                                    className="flex items-center gap-2 text-[#ff895d] font-bold text-sm lg:text-[15px] hover:opacity-80 transition-opacity w-fit"
                                >
            ...
            <div className="flex flex-col gap-8 lg:gap-10">
                <button 
                    ...
                    className="flex items-center gap-3 text-[#ff895d] font-bold text-[15px] lg:text-base hover:opacity-80 transition-opacity w-full lg:w-fit lg:ml-auto justify-center lg:justify-start"
                >
